...
if [[ -z "${API_BASE_URL:-}" ]]; then
  if RESP=$(curl -sf --max-time 2 "http://127.0.0.1:5046/api/tours/public?pageSize=1" 2>/dev/null) \
     && [[ "$RESP" == *'"data"'* || "$RESP" == *'"total"'* ]]; then
    API_URL="http://127.0.0.1:5046"
    echo "Phát hiện Gateway HTTP :5046."
  elif RESP=$(curl -ksf --max-time 2 "https://127.0.0.1:7010/api/tours/public?pageSize=1" 2>/dev/null) \
     && [[ "$RESP" == *'"data"'* || "$RESP" == *'"total"'* ]]; then
